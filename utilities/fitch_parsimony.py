    ...
    def fill_transition_DP(v, s, s1, s2, obs_transitions, unique):
        ...
        for i, u in zip(range(len(children)), children):
            ...
            for sp in LS[i]:
                if not unique:
                    A[i] += C[
                        node_to_i[u], label_to_j[sp], label_to_j[s1], label_to_j[s2]
                    ]
                else:
                    A[i] += min(
                        1,
                        C[node_to_i[u], label_to_j[sp], label_to_j[s1], label_to_j[s2]],
                    )
    ...
    for n in nx.dfs_postorder_nodes(T, source=root):
        for s in T.nodes[n]["label"]:
            for s_pair in itertools.product(possible_labels, repeat=2):
                ...
                ] = fill_transition_DP(n, s, s1, s2, obs_transitions, count_unique)
...
def fitch_bottom_up(tree, root):
    ...
    while d >= 0:
        ...
        for i in internal_nodes:
            ...
            _intersect = reduce(
                np.intersect1d, [tree.nodes[c]["label"] for c in children]
            )
            if len(_intersect) > 0:
                tree.nodes[i]["label"] = _intersect
            else:
                tree.nodes[i]["label"] = reduce(
                    np.union1d, [tree.nodes[c]["label"] for c in children]
                )
