...
def compute_RNA_corr(counts, cells, _method = "euclidean"):
	...
	if _method == "frobenius":
		...
		corrs = sp.spatial.distance.pdist(entries, metric='minkowski', p=2.)
...
if __name__ == "__main__":
	...
	parser.add_argument('--expr', type=str, help='Expression matrix for RNA correlatoins. Assumed to be Cells x Components (genes, PCs, etc.)')
	parser.add_argument("--meta", type=str, help='Meta file')
	parser.add_argument("--meta_item", type=str, help='Column in the meta file to assign to cells.')
	parser.add_argument("--inter_meta", action='store_true', default=False, help='compute correlations between cells of different meta assignment')
	parser.add_argument("--intra_meta", action='store_true', default=False, help='compute correlations between cells of the same meta assignment')
	parser.add_argument("--out_fp", default=None, type = str, help='Write out correlations to sparse matrix format at the specified file location.')
	parser.add_argument("--rna_comp_method", default='euclidean', type=str, help='Choose from euclidean, frobenius, or pearson')
	...
	if out_fp:
		...
		if expr_fp:
			...
			plt.title("Pearson Corr: " + str(st.stats.pearsonr(tree_dists, rna_dists)[0]))
			...
			plt.ylabel("RNA Distance, " + _method)
			plt.title("Pearson Corr: " + str(st.stats.pearsonr(edit_dists, rna_dists)[0]))
